...
namespace Facepunch.Forsaken.FlowFields
{
    ...
    public class FlowField : IIntegrationContainer
    {
        ...
        private readonly Queue<int> FlowQueue = new();
        private readonly List<int> FlowsToBeProcessed = new();
        private readonly Dictionary<int, int[]> PreviousFlows = new();
        private readonly Dictionary<Gateway, Gateway> PreviousGatewayPath = new();

        private Vector3 InternalDestinationPosition;
        ...
        public virtual Vector3 DestinationPosition => InternalDestinationPosition;
        ...
        public void CreateDestinationGateways( List<int> worldIndexes )
        {
            ...
            InternalDestinationPosition = Pathfinder.GetPosition( Pathfinder.CreateWorldPosition( DestinationIndex ) );
        ...
        protected virtual bool SeekPath( Vector3 startPosition )
        {
            ...
            if ( DestinationGateways.ContainsKey( worldPosition.ChunkIndex ) )
            {
                ...
                FlowQueue.Enqueue( firstGateway.Chunk );
        ...
        private void BuildIntegrationStack( Gateway fromGateway, Gateway untilGateway )
        {
            while ( !Equals( fromGateway, untilGateway ) )
            {
                ...
                FlowQueue.Enqueue(fromGateway.Chunk);
            ...
            if ( !RestoreFromCache( untilGateway ) )
                FlowQueue.Enqueue( untilGateway.Chunk );
        }

        private bool RestoreFromCache( Gateway connectionGateway )
        {
            if ( !PreviousGatewayPath.ContainsKey( connectionGateway ) || !GatewayPath.ContainsKey( connectionGateway ) )
                return false;

            if ( !PreviousGatewayPath[connectionGateway].Equals( GatewayPath[connectionGateway] ) )
                return false;

            Flows[connectionGateway.Chunk] = PreviousFlows[connectionGateway.Chunk];
            return true;
        }
        ...
        private void QueueFlow()
        {
            ...
            {
                var chunkIndex = FlowQueue.Dequeue();
                if ( Flows.ContainsKey( chunkIndex ) ) continue;

                if ( FlowsToBeProcessed.Contains( chunkIndex ) )
                    continue;

                FlowsToBeProcessed.Add( chunkIndex );
        ...
        public void CalculateFlow( int chunkIndex )
        {
            ...
            FlowsToBeProcessed.Remove(chunkIndex);
        }
        ...
        {
            if ( FlowsToBeProcessed.Contains( position.ChunkIndex ) )
