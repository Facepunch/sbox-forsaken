...
using Facepunch.Forsaken.FlowFields.Entities;
...
namespace Facepunch.Forsaken.FlowFields
{
    public class Pathfinder
    {
		...
		public void SetCollisionTags( params string[] tags )
		{
			CollisionTags = tags;
		}
		...
		public TraceResult GetCollisionTrace( Vector3 position, int worldIndex )
		{
			...
			transform.Position = (position + InternalCenterOffset).WithZ( InternalCollisionExtents.z + heightMap + 12f );

			var trace = Trace.Sweep( InternalPhysicsBody, transform, transform )
				.WorldAndEntities()
				.WithAnyTags( CollisionTags )
				.WithoutTags( "ff_ignore", "passplayers" )
				.Run();
        ...
        {
			if ( InternalCollisionBuffer.Count > 0 )
			{
				Log.Info( "UPDATE COLL: " + InternalCollisionBuffer.Count );
			}
        ...
        public GridWorldPosition CreateWorldPosition( Vector3 position )
        {
			...
			return CreateWorldPosition( GridUtility.GetIndex( WorldGridSize, ( position.y / NodeSize ).FloorToInt(), ( position.x / NodeSize ).FloorToInt() ) );
        }
