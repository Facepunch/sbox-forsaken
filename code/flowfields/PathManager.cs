...
namespace Facepunch.Forsaken.FlowFields
{
    public static class PathManager
    {
		...
		public static void UpdateCollisions( Vector3 position, float radius = 256f )
		{
			foreach ( var pathfinder in All )
			{
		...
		{
			var hash = HashCode.Combine( (short)nodeSize, (short)collisionSize );
		...
		public static async Task Create( int nodeSize = 50, int collisionSize = 100, params string[] collisionTags )
		{
			if ( !Bounds.HasValue )
			...
			{
				var pathfinder = new Pathfinder( Bounds.Value, nodeSize, collisionSize );
				pathfinder.SetCollisionTags( collisionTags );
				await Register( pathfinder, nodeSize, collisionSize );
			}
		}
